...
interface CardListSubModulesProps {
  data: SubModules[];
  isPending?: boolean;
}

export const CardListSubModulesSkeleton = () => {
  return (
    <>
...
export default function CardListSubModules({
  data,
  isPending,
}: CardListSubModulesProps) {
  if (isPending) {
    return <CardListSubModulesSkeleton />;
  }
    ...
    <div className="grid grid-cols-1 gap-6">
      {data?.length === 0 ? (
        <div className="text-center text-muted-foreground">
          Sub Modul tidak ditemukan.
        </div>
      ) : (
        data?.map((subModule) => (
          <Link
            href={`/dashboard/modules/${subModule.id}/contents`}
            key={subModule.id}
